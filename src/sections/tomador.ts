import { NfseCoreData } from "../types";
import { mm, pxToMm } from "../utils/formatters";
import { LAYOUT_CONSTANTS } from "../utils/layout-constants";

function getEnderecoTomador(
  toma: NfseCoreData["tomador"],
  localEmissao: string,
  localPrestacao: string,
): [string, string] {
  let cidade = toma.municipio || null;
  if (cidade && toma.uf) {
    cidade = `${toma.municipio} - ${toma.uf}`;
  } else {
    cidade =
      localPrestacao && localPrestacao !== localEmissao
        ? localPrestacao
        : localEmissao;
  }

  const complemento = toma.complemento ? `, ${toma.complemento}` : "";
  const endereco = `${toma.logradouro}, ${toma.numero}${complemento}, ${toma.bairro}`;

  return [cidade || "", endereco];
}

export function renderTomador(
  pdf: PDFKit.PDFDocument,
  data: NfseCoreData,
  font: string,
): void {
  const { col1X, col2X, col3X, col4X, col1W, col2W, col3W, col4W } =
    LAYOUT_CONSTANTS.columns;

  const toma = data.tomador;
  const y0 = pxToMm(pdf.y - 8);

  pdf.font("Arial-Semibold").fontSize(8);
  pdf.text("TOMADOR DO SERVIÇO", mm(col1X), mm(y0), { width: mm(col1W) });
  pdf.text("CNPJ / CPF / NIF", mm(col2X), mm(y0), { width: mm(col2W) });
  pdf.text("Inscrição Municipal", mm(col3X), mm(y0), { width: mm(col3W) });
  pdf.text("Telefone", mm(col4X), mm(y0), { width: mm(col4W) });

  const y1 = y0 + 5;
  pdf.font(font).fontSize(8);
  pdf.text("", mm(col1X), mm(y1), { width: mm(col1W) });
  pdf.text(toma.doc, mm(col2X), mm(y1), { width: mm(col2W) });
  pdf.text(toma.inscricaoMunicipal, mm(col3X), mm(y1), { width: mm(col3W) });
  pdf.text(toma.fone, mm(col4X), mm(y1), { width: mm(col4W) });

  const y2 = y1 + 5;
  pdf.font("Arial-Semibold").fontSize(8);
  pdf.text("Nome / Nome Empresarial", mm(col1X), mm(y2), {
    width: mm(col1W),
  });
  pdf.text("E-mail", mm(col3X), mm(y2), { width: mm(col3W) });

  const y3 = y2 + 4;
  pdf.font(font).fontSize(8);
  pdf.text(toma.nome, mm(col1X), mm(y3), { width: mm(col1W + col2W) });
  pdf.text(toma.email, mm(col3X), mm(y3), { width: mm(col3W + col4W) });

  const y4 = y3 + 5;
  pdf.font("Arial-Semibold").fontSize(8);
  pdf.text("Endereço", mm(col1X), mm(y4), { width: mm(col1W) });
  pdf.text("Município", mm(col3X), mm(y4), { width: mm(col3W) });
  pdf.text("CEP", mm(col4X), mm(y4), { width: mm(col4W) });

  const [tomadorCidade, tomadorEndereco] = getEnderecoTomador(
    toma,
    data.localEmissao,
    data.localPrestacao,
  );
  const y5 = y4 + 4;
  pdf.font(font).fontSize(8);
  pdf.text(tomadorEndereco, mm(col1X), mm(y5), {
    width: mm(col1W + col2W),
  });
  pdf.text(tomadorCidade, mm(col3X), mm(y5), { width: mm(col3W) });
  pdf.text(toma.cep, mm(col4X), mm(y5), { width: mm(col4W) });

  pdf.y = mm(y5 + 6);
}
